...
def get_order_by_client_id(client_id: int):
    try:
        ...
        client_orders = session.query(ClientOrderModel).filter_by(client_id= client_id).all()
        all_client_orders = []

        for order in client_orders:
            products = []

            product_orders = session.query(ProductOrderModel).filter_by(order_id= order.id).all()
