...
from app.exceptions.client_exc import ClientNotFound, DuplicateProduct, ProductNotFound, UnavailableProduct, WrongKeys


from app.models import ClientsModel, ProductModel, OrderModel
from app.configs.database import db
from app.services.clients_services import checking_id, checking_keys, calculate_price, register_products_order, register_client_order, checking_duplicate, packing_products
...
def create_checkout(client_id: int):
    try:
        ...
        return {'error': 'Produto não disponível ou demanda excedente ao estoque'}, HTTPStatus.UNPROCESSABLE_ENTITY
    
    except ClientNotFound:
        return {'error': 'Cliente não encontrado, verifique o id'}, HTTPStatus.NOT_FOUND
    
    except DuplicateProduct:
        return {'error': 'Produto pedido mais de uma vez na mesma compra, considere incrementar a quantidade'}, HTTPStatus.BAD_REQUEST
